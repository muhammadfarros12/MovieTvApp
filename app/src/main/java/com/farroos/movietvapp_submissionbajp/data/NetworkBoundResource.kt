...
import com.farroos.movietvapp_submissionbajp.data.source.remote.StatusResponse
            ...
            when (response.statusResponse) {
                StatusResponse.SUCCESS ->
                    CoroutineScope(IO).launch {
                        ...
                        Log.d("BOUND 1: ", response.statusResponse.name)
                ...
                StatusResponse.ERROR -> {
                    onFetchFailed()
                    Log.d("Bound 2 : ", response.statusResponse.name)
                    result.addSource(dbSource) { newData ->
